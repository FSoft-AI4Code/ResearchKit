...
def verify_by_arxiv_id(arxiv_id: str, expected_title: str) -> CitationResult | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "ResearchKit/0.1"})
...
def verify_by_doi(doi: str, expected_title: str) -> CitationResult | None:
    ...
    try:
        req = urllib.request.Request(
            url,
            headers={
                "User-Agent": "ResearchKit/0.1 (mailto:researchkit@example.com)",
                "Accept": "application/json",
            },
        )
...
_OPENALEX_EMAIL = "researchkit@users.noreply.github.com"


def verify_by_openalex(
    title: str,
    *,
    openalex_email: str = _OPENALEX_EMAIL,
) -> CitationResult | None:
    ...
    try:
        req = urllib.request.Request(
            url,
            headers={
                "User-Agent": f"ResearchKit/0.1 (mailto:{openalex_email})",
                "Accept": "application/json",
            },
        )
    ...
    if best_sim >= 0.80:
        ...
        return CitationResult(
            ...
            method="openalex",
            details=(
                f"Partial match via OpenAlex (sim={best_sim:.2f}): "
...
_CACHE_DIR = Path.home() / ".cache" / "researchkit" / "citation_verify"
...
def verify_by_title_search(
    ...
    *,
    ...
    """
    from researchkit.literature.search import search_papers
    ...
    *,
    ...
    for i, entry in enumerate(entries):
        ...
        if result is None:
            ...
            result = verify_by_openalex(title, openalex_email=openalex_email)
            api_call_count += 1
            if result is not None:
                logger.info(
